...
class OTPScreen extends ConsumerStatefulWidget {
  ...
  const OTPScreen(
      {super.key,
      this.typeOfMode = TypeOfMode.ViewMode,
      required this.phone,
...
class _OtpScreenState extends ConsumerState<OTPScreen> {
  ...
  Widget build(BuildContext context) {
    handleState(sendOtpForLoginStateProvider,
        showLoading: true, showToast: true, onSuccess: (state) {
      timerKey.currentState?.restart();
    });

    handleState(confirmResetCodeStateProvider, showToast: true,
        onSuccess: (state) {
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  InkWell(
                      child: SVGIcons.editIcon(color: AppTheme.appGrey7),
                      onTap: () {
                        context.pop();
                      }),
                  const SizedBox(
              ...
              Padding(
                  ...
                  child: AppButton(
                    onPress: () {
                      if (otpFieldsKeys.currentState?.formKey.currentState
                              ?.validate() ==
                          true) {
                        verifyPhone(
                            widget.phone, otpFieldsKeys.currentState?.getCode);
                      }
                    },
                    text: context.tr(verifyKey),
                    height: 48,
  ...
  void initFcmToken() async {
    ...
    ref
        .read(updateFcmTokenStateProvider.notifier)
        .calculateInstantOrder(fcmToken: fcmToken);
  }
  ...
  void login() async {
    ...
    if (sessionId?.isNotEmpty == true) {
      ref.read(loginStateNotifierProvider.notifier).login(widget.phone,
          countryCode: widget.codeCountry, sessionId: sessionId);
    } else {
      ref
          .read(loginStateNotifierProvider.notifier)
          .login(widget.phone, countryCode: widget.codeCountry);
    }
  }
