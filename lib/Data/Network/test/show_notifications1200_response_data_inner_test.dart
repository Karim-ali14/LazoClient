//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:lazo/api.dart';
import 'package:test/test.dart';

// tests for ShowNotifications1200ResponseDataInner
void main() {
  // final instance = ShowNotifications1200ResponseDataInner();

  group('test ShowNotifications1200ResponseDataInner', () {
    // String createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // num id
    test('to test the property `id`', () async {
      // TODO
    });

    // String image
    test('to test the property `image`', () async {
      // TODO
    });

    // String imagePath
    test('to test the property `imagePath`', () async {
      // TODO
    });

    // num isRead
    test('to test the property `isRead`', () async {
      // TODO
    });

    // String type
    test('to test the property `type`', () async {
      // TODO
    });

    // num orderId
    test('to test the property `orderId`', () async {
      // TODO
    });

    // String notification
    test('to test the property `notification`', () async {
      // TODO
    });

    // String notificationAr
    test('to test the property `notificationAr`', () async {
      // TODO
    });

    // String notificationEn
    test('to test the property `notificationEn`', () async {
      // TODO
    });

    // ShowNotifications1200ResponseDataInnerProvider provider
    test('to test the property `provider`', () async {
      // TODO
    });

    // num providerId
    test('to test the property `providerId`', () async {
      // TODO
    });

    // String title
    test('to test the property `title`', () async {
      // TODO
    });

    // String titleAr
    test('to test the property `titleAr`', () async {
      // TODO
    });

    // String titleEn
    test('to test the property `titleEn`', () async {
      // TODO
    });

    // String updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // ShowNotifications1200ResponseDataInnerUser user
    test('to test the property `user`', () async {
      // TODO
    });

    // Object userId
    test('to test the property `userId`', () async {
      // TODO
    });


  });

}
