...
class ProviderApi {
  ...
  ///
  /// * [String] page:
  Future<Response> showtransactionsWithHttpInfo({ String? dateFrom, String? dateTo, String? page, }) async {
    // ignore: prefer_const_declarations
    final path = r'/provider/transactions';
  ...
  ///
  /// * [String] page:
  Future<Showtransactions200Response?> showtransactions({ String? dateFrom, String? dateTo, String? page, }) async {
    final response = await showtransactionsWithHttpInfo( dateFrom: dateFrom, dateTo: dateTo, page: page, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'Showtransactions200Response',) as Showtransactions200Response;
