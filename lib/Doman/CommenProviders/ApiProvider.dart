...
extension GenericRequest<T> on StateNotifier<StateModel<T>> {
  ...
  Future<void> requestWithHandleMessage(Future<dynamic> Function() asyncFunc,
      {Function(T)? onComplete, Function(ApiException)? onFailure}) async {
    var response;
    try {
      response = await asyncFunc();
    ...
    ref.listen(provider, (previous, next) {
      ...
      next.handelStateWithoutWidget(
        ...
        if (showToast == true) {
          AppSnackBar.showSnackBar(context,
              isSuccess: false,
              message: state.message ?? context.tr("somethingWrong"));
        }
        ...
        if (alertKey != null) {
          ...
          alertKey.currentState?.showAlert(
              message: state.message?.replaceAll("Alert:", "") ??
                  context.tr("somethingWrong"),
              alertType:
...
extension RequestHandle2<T> on ConsumerWidget {
  void handleState<T>(WidgetRef ref, BuildContext context,
      AutoDisposeStateNotifierProvider<dynamic, StateModel<T>> provider,
      {bool? showLoading,
      bool? showToast,
      GlobalKey<SimpleAlertState>? alertKey,
